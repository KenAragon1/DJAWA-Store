    ...
    <div class="flex flex-col px-4 py-10 mx-auto my-8 overflow-hidden bg-white border shadow-xl w-96">
        ...
        <form class="flex flex-col w-11/12 mx-auto mb-4" method="POST" action="{{ route('register') }}">
            ...
            <div class="w-full">
                ...
                @error('username')
                    <span class="text-sm text-red-800 "><i class="fa-solid fa-circle-exclamation"></i>
                        {{ $message }}</span>
                @enderror

            </div>
            <div class="w-full mt-8 ">
                <input class="w-full px-4 py-3 rounded outline outline-1 outline-slate-500" type="email" name="email"
                    placeholder="Email">
                @error('email')
                    <span class="text-sm text-red-800 "><i class="fa-solid fa-circle-exclamation"></i>
                        {{ $message }}</span>
            ...
            <div class="relative w-full mt-8">
                <input class="w-full px-4 py-3 rounded outline outline-1 outline-slate-500" type="password"
                    id="password-input" placeholder="Confirm Password">
            </div>
            <button class="py-4 mt-8 text-white transition-all rounded-lg bg-pastel-blue hover:bg-dark-pastel-blue"
